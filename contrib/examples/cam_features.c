...
{   dc1394camera_t *pCamera, **pCameras=NULL;
    ...
    //EXIF STUFF
    char cJpegOutputname[] = "foobar2.jpg";
    JPEGData *pData;
    //float fOnefloat;
    ExifData * pEd;
  ...
  free(pCameras);
    ...
    if(dc1394_get_camera_feature_set(pCamera, &xFeatures)!=DC1394_SUCCESS) 
            fprintf(stdio, "unable to get feature set\n");
    else
    ...
    pData = jpeg_data_new_from_file ("test.jpg");  //input data
    if (!pData) {
...
/*
    pData = jpeg_data_new_from_file ("test.jpg");  //input data
    if (!pData) {//need to create logic according to the value of the sharpness
        printf ("Could not load '%s'!\n", "test.jpg");
        return (1);
    }
